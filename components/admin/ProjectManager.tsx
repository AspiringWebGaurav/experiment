...
export default function ProjectManager() {
  ...
  const addIconField = () => {
    ...
    if (currentIcons.length < MAX_ICON_LISTS) {
      ...
      toast.error(`Maximum ${MAX_ICON_LISTS} technology icons allowed`);
    }
  };
  ...
  const handleIconSelect = (iconUrl: string) => {
    ...
    if (emptyIndex !== -1) {
      ...
      toast.error(`Maximum ${MAX_ICON_LISTS} icons allowed`);
    }
  };
  ...
  const startEdit = (id: string) => {
    ...
    try {
      ...
      toast.error("Unable to open edit form. Please try again.");
    }
  };
