  ...
  const saveVersion = async () => {
    ...
    try {
      ...
      await createNotification({
        ...
        message: `Version ${version.trim()} has been successfully created`,
      });

      toast.success(`Version ${version.trim()} created successfully!`);
      setVersion("");
      setChangelog([]);
        ...
        <div className="flex-1 overflow-y-auto space-y-2 scrollbar-thin">
          {history.length === 0 ? (
            ...
            history.map((item) => (
              <div
                key={item.id}
                className="p-3 rounded bg-gray-50 border border-gray-200 hover:bg-gray-100 transition-colors"
              >
