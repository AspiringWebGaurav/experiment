...
export default function WorkExperienceManager() {
  ...
  const handleUrlSubmit = () => {
    ...
    if (!tempUrl.match(/^(https?:\/\/|\/)/)) {
      toast.error("Please enter a valid HTTP/HTTPS URL or path");
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to delete "${experience.title}"? It will be moved to the recycle bin.`
    );
    ...
    try {
      ...
      if (
        ...
        toast.success("Deleted image from cloud");
      }
  ...
  const moveWorkExperience = async (id: string, direction: "up" | "down") => {
    ...
    try {
      ...
      toast.error("Failed to reorder work experiences");
    }
  };
